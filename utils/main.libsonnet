...

{
  _withArrayItem(arr, override, matcher, mixin=false)::
    std.mapWithIndex(
      function(index, item)
        if matcher == '*'
        then overrideValue(item, override, mixin)
  ...
  withArrayItemAtPath(path, override, matcher, mixin=false, depth=0)::
    ...
    then { [key]+: $.withArrayItemAtPath(path, override, matcher, mixin, (depth + 1)) }
    else
      if mixin == true
      then { [key]: $._withArrayItemMixin(super[key], override, matcher) }
      else { [key]: $._withArrayItem(super[key], override, matcher) },
  ...
  withValueAtPath(path, override, mixin=false, depth=0)::
    ...
    local key = pathArray[depth];
    if depth < std.length(pathArray) - 1
    then { [key]+: $.withValueAtPath(path, override, mixin=mixin, depth=(depth + 1)) }
    else
      if mixin == true
      then { [key]+: override }
      else { [key]: override },
